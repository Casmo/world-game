...
/**
 * A player works a built production Building: a Work Activity (a timed shift,
 * ADR-0007) that, on completion, produces Resources for the Team and earns the
 * worker Experience in that Building's trade. Bounded by the Building's
 * work-slots and gated on Energy, like construction.
 */
class StartWork
{
    /**
    ...
    public function handle(User $user, Building $building): Activity
    {
        return DB::transaction(function () use ($user, $building) {
            // Lock the Building so concurrent workers can't exceed work-slots (ADR-0010).
            ...
            if (! $building->hasFreeProductionSlot()) {
                throw new WorkSlotsFullException;
            }

            if ($user->currentEnergy() < Building::WORK_ENERGY_COST) {
                throw new InsufficientEnergyException;
            }

            $user->forceFill([
                'energy' => $user->energy - Building::WORK_ENERGY_COST,
            ])->save();
            ...
            return $user->activities()->create([
                ...
                'started_at' => now(),
                'completes_at' => now()->addSeconds(ActivityType::Work->durationSeconds()),
            ]);
        });
    }
}
